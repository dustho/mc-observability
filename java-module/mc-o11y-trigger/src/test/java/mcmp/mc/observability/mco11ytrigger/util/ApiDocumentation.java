...
public final class ApiDocumentation {
	...
	public static HeaderDescriptorWithType header(String name, ParameterType type, String description) {
		return headerWithName(name).type(type.toSimpleType()).description(description);
	}

	public static ParameterDescriptorWithType parameter(String name, ParameterType type, String description) {
	...
	public static FieldDescriptor fieldEnum(String path, String description, Class<? extends Enum<?>> clazz) {
		return fieldWithPath(path).type(ENUM_TYPE).description(description)
				.attributes(key(ENUM_KEY).value(Arrays.asList(clazz.getEnumConstants())));
	}

	public static FieldDescriptor fieldSubsection(String path, JsonFieldType type, String description) {
	...
	public static class ApiDocumentationBuilder {
		...
		public ApiDocumentationBuilder queryParameters(ParameterDescriptorWithType... queryParameters) {
		...
		public RestDocumentationResultHandler build() {
			return MockMvcRestDocumentationWrapper.document("{method-name}", preprocessRequest(prettyPrint()),
					preprocessResponse(prettyPrint()),
					resource(ResourceSnippetParameters.builder().tag(tag).description(description)
							.requestSchema(requestSchema).responseSchema(responseSchema).requestHeaders(requestHeaders)
							.responseHeaders(responseHeaders).pathParameters(pathParameters)
							.queryParameters(queryParameters).requestFields(requestFields)
	...
	public enum ParameterType {
		STRING(SimpleType.STRING), INTEGER(SimpleType.INTEGER), NUMBER(SimpleType.NUMBER), BOOLEAN(SimpleType.BOOLEAN);
