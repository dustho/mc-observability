...
public class GrafanaAlertManager implements AlertManager {
  ...
  public GrafanaAlertManager(
      GrafanaClient grafanaClient, GrafanaAlertRuleFactory alertRuleFactory) {
  ...
  public void createAlertRule(AlertRuleCreateDto dto) {
    String query =
        Flux.from("cmp/autogen")
            .range(-2L, -1L, ChronoUnit.MINUTES)
            .filter(
                Restrictions.and(
                    Restrictions.measurement().equal(dto.getMeasurement()),
            ...
            .groupBy(new String[] {"target_id", "ns_id"})
            .toString();
